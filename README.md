...
| 🧬 [**Smart Asset Clustering**](docs/Smart_Asset_Clustering_Illustrated_Manual.pdf) | Group generations by node architecture or prompt text (`Shift+C`) using colorful `#HASH` badges for at-a-glance visual grouping - **NEW** |
...
- 🧬 [**Smart Asset Clustering & Visual Inspector (`Shift+C`)**](docs/Smart_Asset_Clustering_Illustrated_Manual.pdf): Instantly group and compare media generated by identical ComfyUI graph architectures (`workflow_hash`) or identical positive prompts (`prompt_hash`). Click any colorful `#HASH` badge to open the interactive **Cluster Inspector** to view node pipeline chains, loaded model inventories, and launch one-click cluster searches. 👉 READ the **[Smart Asset Clustering Manual](docs/Smart_Asset_Clustering_Illustrated_Manual.pdf)**.
...
👉 **[Read the Smart Asset Clustering Manual](docs/Smart_Asset_Clustering_Illustrated_Manual.pdf)**
